...
# def make_tool_proxy(tool_name: str, mcp):
#     async def _tool_fn(*args):
#         # The function_wrapper now includes performance logging
#         result = await mcp.function_wrapper(tool_name, *args)
        
#         # Handle CallToolResult objects by ensuring they can be properly converted
#         if hasattr(result, 'content') and hasattr(result, 'isError'):
#             try:
#                 # Try to extract the actual value from the result object
#                 if hasattr(result, '__str__'):
#                     string_val = str(result).strip()
#                     # Try to convert to a native Python type if possible
#                     try:
#                         # If it looks like a number, convert it
#                         if string_val.replace('.', '').replace('-', '').isdigit():
#                             if '.' in string_val:
#                                 return float(string_val)
#                             else:
#                                 return int(string_val)
#                         # If it looks like a boolean, convert it
#                         elif string_val.lower() in ('true', 'false'):
