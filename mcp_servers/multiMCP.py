...
import ast
from memory.tool_performance_log import ToolPerformanceLogger
...
class MultiMCP:
    def __init__(self, server_configs: List[dict]):
        ...
        # Initialize the tool performance logger
        self.performance_logger = ToolPerformanceLogger()
                    ...
                    try:
                        async with ClientSession(read, write) as session:
                            ...
                            for tool in tools.tools:
                                ...
                                # Register tool with the performance logger
                                self.performance_logger.register_tool(
                                    tool_name=tool.name,
                                    server_id=server_key,
                                    description=tool.description or f"Tool {tool.name} from {server_key}"
                                )
    ...
    async def call_tool(self, tool_name: str, arguments: dict) -> Any:
        ...
        try:
            ...
            self.performance_logger.record_tool_call(
                tool_name=tool_name,
                server_id=config["id"],
            ...
            self.performance_logger.record_tool_call(
                tool_name=tool_name,
                server_id=config["id"],
                call_result={"status": "error", "error": str(e)},
                response_time=response_time
            )
    ...
    def get_tool_performance_summary(self) -> Dict[str, Any]:
        """
        Get a summary of tool performance metrics
        
        Returns:
            Dictionary with tool reliability metrics and recommendations
        """
